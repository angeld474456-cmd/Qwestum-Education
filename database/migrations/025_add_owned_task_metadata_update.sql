-- Sprint 12.20.28A: atomically update metadata and content for one owned task.
-- Image, media, ordering, task type, and quest membership remain outside this
-- boundary until their dedicated mutation paths are migrated.

CREATE FUNCTION public.update_owned_quest_task_content(
  p_quest_id uuid,
  p_task_id uuid,
  p_title text,
  p_description text,
...
BEGIN
  ...
  IF v_task_type = 'single_choice' THEN
    IF p_content IS NULL
      OR pg_catalog.jsonb_typeof(p_content -> 'options') IS DISTINCT FROM 'array'
  ...
  IF v_task_type = 'multiple_choice' THEN
    IF p_content IS NULL
      OR pg_catalog.jsonb_typeof(p_content -> 'options') IS DISTINCT FROM 'array'
