      ...
      AND (
        ...
        OR (p.polrelid = 'public.quest_tasks'::pg_catalog.regclass
          AND (p.polname <> 'Teachers can select tasks for own quests' OR p.polcmd <> 'r' OR p.polroles <> ARRAY[v_authenticated_role]::oid[] OR p.polwithcheck IS NOT NULL
            OR lower(regexp_replace(regexp_replace(regexp_replace(pg_catalog.pg_get_expr(p.polqual, p.polrelid), '[[:space:]()]', '', 'g'), '::text', '', 'g'), 'from(public[.])?quests(as)?parent_quest', 'frompublic.questsparent_quest', 'gi')) <> lower(regexp_replace(regexp_replace(regexp_replace($task_select$
              exists (
                select 1
            ...
            $task_select$, '[[:space:]()]', '', 'g'), '::text', '', 'g'), 'from(public[.])?quests(as)?parent_quest', 'frompublic.questsparent_quest', 'gi'))))
      )
