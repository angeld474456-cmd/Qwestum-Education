...
Only the environment variable names `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`, and `RATE_LIMIT_HMAC_SECRET` are used. Sprint 12.20.18 verified the limiter against a separate Preview Upstash database from a controlled Vercel Preview deployment; values remain provider-managed and undocumented. Sprint 12.20.32 completed controlled authorized Preview enforcement evidence: normal submit returned `200`, then same-identity/same-quest sequential requests returned the first fixed no-store `429` on attempt 62 with `Retry-After: 15` and no `503`. No timeout adjustment was made or is currently required by this evidence. Sprint 12.20.33 verified that all application runtime variables are currently Preview-only; Production limiter configuration therefore remains a P0 blocker.
