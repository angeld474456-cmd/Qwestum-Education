...
- Recommended MVP is anonymous `/catalog` and `/catalog/[id]` browsing, login before later `/catalog/[id]/start`, separate public/student DTOs, and no base authoring-row exposure. Keep `is_public`; defer states, timestamps, versioning, moderation, slugs, payments, assignments, entitlements, and attempts. Cards initially need optional cover, title, short description, difficulty, grade, language, subject, and action; server-side URL filters are text, subject, grade, difficulty, and language. Covers are public but paths contain owner UUIDs, so return only validated URLs with fallback and require explicit disclosure acceptance.
- Public DTO allowlist is public ID, title, description, difficulty, subject display data, grade, language, optional duration/category/tags, and validated cover URL. Deny author/ownership metadata, raw paths, unpublished state, answer/hint/`correctOptionId`, scoring rules, raw task JSON, notes, and implementation errors. Use dynamic or short-revalidated server reads, generic failures, indistinguishable missing/draft 404s, safe local login returns only, and no static unrestricted cache. Sprint 12.19.2 is read-only live-schema/public-boundary verification; no migration, policy, catalog code, or SQL is authorized.
