...
Only the environment variable names `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`, and `RATE_LIMIT_HMAC_SECRET` are used. Sprint 12.20.18 verified the limiter against a separate Preview Upstash database from a controlled Vercel Preview deployment; values remain provider-managed and undocumented. Production credentials, Vercel WAF/rate rules, controlled `429` verification, and production deployment remain incomplete.
...
Live shared-limiter `429` evidence remains deferred to pre-production verification. Codex execution-context requests received Vercel-marked redirects for anonymous public GETs and external `401` for public submit, while the application route and proxy have no matching `401` path and browser Preview access was authorized. This identifies external Preview/Deployment Protection as the boundary and does not indicate a limiter defect. Do not weaken Preview protection to collect this evidence. No Production promotion, artifact cleanup, aliasing, rollback, or provider mutation occurred; the initial Production deployment remains a provisioning artifact. The next boundary is planning the controlled pre-production checklist, including an authorized-context `429` method, rollback and artifact disposition, proxy/cache review, and remaining Vercel WAF/Firewall assessment.
