...
The next boundary is Preview auth, rate-limit, and deployment-safety verification: determine the safe disposition of the accidental Production artifact, define the exact Supabase Auth Preview redirect allowlist, verify magic-link/login and teacher/dashboard flows, execute a controlled `429` check, and complete the remaining deployment/security review. Each external configuration or verification action remains separately approval-gated.
