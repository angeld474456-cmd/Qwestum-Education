...
The current Production rollback baseline is Vercel deployment `dpl_146uK8UYRdnFZFPGyDfrKXsfpG4Y` at `qwestum-education-gskc2mem9-qwestum.vercel.app`, Ready/Current in Production from `main` commit `3e500e2642c19252b8c4d79bd40964c4a6f21e81`. The earlier `bfe773a` deployment affected by the malformed Supabase URL is not a rollback target. For an urgent operational rollback, promote the recorded known-good Vercel deployment; for a normal code regression, use a reviewed `git revert` on `main`; never force-push/reset `main`. Neither deployment nor Git rollback reverses database/data state, and schema remains frozen through Migration 035. Current Hobby/free observability is manual through Vercel deployment status, runtime logs, and basic Observability. Watch public entry/catalog/runtime/submit and auth/dashboard/logout routes; repeated 5xx, unexpected auth failures, catalog/runtime RPC failures, and submit `503` require investigation, while threshold `429` remains expected limiter behavior.
