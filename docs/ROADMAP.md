...
- Public language-label polish is complete. The next launch-critical planning item is **P1 Program-Aware Teacher Subject Lookup Planning**: before adding a second education profile, define an authenticated-safe subject selector boundary that cannot mix offerings across programs while preserving the current no-broad-taxonomy-read RLS model. No implementation is approved by this roadmap entry.
