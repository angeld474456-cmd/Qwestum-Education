...
- Live, policy-verified Migration 024 removes the legacy direct authenticated `public.quest_tasks` DELETE policy, `Teachers can delete tasks for own quests`. RLS remains enabled: SELECT and UPDATE policies are present; INSERT and DELETE policies are absent. The DELETE route now delegates only through `services/teacher-task-deletion.server.ts`, which calls the RPC once with only `p_quest_id` and `p_task_id` and strictly allowlists exact outcome shapes.
...
- Image-only PATCH and compare-and-clear image removal intentionally remain direct while their dedicated image mutation boundary is pending. The direct UPDATE policy remains required. Replacement cleanup is best-effort after the committed image update, including thrown cleanup exceptions, which cannot change success into HTTP 500.
