...
- Public quest language labels now use the shared Russian label map rather than raw stored codes. The next planning handoff is **P1 Program-Aware Teacher Subject Lookup Planning**; do not expand into student identity, attempts, payments, assignments, organizations, AI, exports, broader localization, or a second taxonomy profile without separate approval.
