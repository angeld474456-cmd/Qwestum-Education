...
- Sprint 12.19.1 - Public Quest Catalog and Student Access Planning.
  ...
  - Verified quest fields are `id`, `title`, nullable `description`, `author_id`, `difficulty`, `is_public`, `created_at`, nullable `subject_id`, `grade_min`, `grade_max`, `estimated_duration_minutes`, nullable `language_code`, nullable `cover_image_path`, nullable `category`, and `tags`. `is_public` is the only publication state: Settings supports publish/unpublish, publishing requires one task, covers and complete-task validation are not required, published quests remain mutable, and legacy task `content` can be null. There is no slug, `published_at`, price/currency, entitlement, author display profile, moderation workflow, or status enum.
  ...
  - Keep `is_public` for catalog MVP; defer `published_at`, enum states, versioning, moderation, and snapshots. Required cards are optional cover, title, short description, difficulty, grade, language, subject, and detail/start action. Duration/category/tags/task count/publication date are deferrable; omit author attribution, price/free-paid, popularity, total points, and payment state. First filters are server-side URL-query text, subject, grade, difficulty, and language; task count needs aggregation and is omitted initially.
  ...
  - `quest-images` is publicly readable and covers can technically render, but paths contain owner UUID segments. Covers remain optional with fallback; explicit acceptance of that disclosure is needed, raw `cover_image_path` stays private, and public DTOs return only validated public URLs. Public responses allow public ID, title, description, difficulty, subject display data, grade, language, optional duration/category/tags, and validated cover URL; they deny author/ownership metadata, raw paths, unpublished internals, answers, hints, `correctOptionId`, scoring/validation rules, raw task content, notes, and error details.
...
- Sprint 12.19.2 - Live Schema and Public Read Boundary Verification.
  - Read-only verification only: inspect live quest/task columns, types, keys, indexes, RLS, functions/views/RPCs, and cover Storage read policy; compare against local migrations/types, identify drift, choose view/RPC/published-policy direction, and produce exact migration plus rollback SQL without applying it.
  - No SQL, migration, view/RPC, policy, catalog code, live-data change, or implementation is authorized without separate approval. Preserve the separate task-workspace QA backlog.
