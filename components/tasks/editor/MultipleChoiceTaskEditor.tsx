...
export default function MultipleChoiceTaskEditor({ task, onSave, onUploadImage, onRemoveImage }: TextTaskEditorProps) {
  ...
  return <div className="mt-8 space-y-6">
    ...
    <div className="space-y-3">{options.map((option) => <div key={option.id} className="grid grid-cols-[auto_minmax(0,1fr)] items-start gap-3 md:grid-cols-[auto_minmax(0,1fr)_auto]"><input type="checkbox" checked={correctOptionIds.includes(option.id)} onChange={() => toggleCorrect(option.id)} aria-label="Правильный ответ" /><textarea rows={3} value={option.text} ref={autoSizeOptionTextarea} onInput={(event) => autoSizeOptionTextarea(event.currentTarget)} onChange={(event) => setOptions((current) => current.map((item) => item.id === option.id ? { ...item, text: event.target.value } : item))} className="min-h-24 min-w-0 w-full resize-y overflow-x-hidden break-words rounded-xl bg-[#1B2435] p-4" aria-label="Вариант ответа" /><button type="button" onClick={() => removeOption(option.id)} className="col-start-2 justify-self-start rounded-lg bg-red-600 px-4 py-3 font-semibold text-white transition hover:bg-red-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-300 focus-visible:ring-offset-2 focus-visible:ring-offset-[#111827] md:col-auto md:justify-self-end md:self-start">Удалить</button></div>)}</div>
